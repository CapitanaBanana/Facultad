...
chan colaNormal(int idPersona, boletas, dinero);
chan colaEmbarazadas(int idPersona, boletas, dinero);
chan colaPocasBoletas(int idPersona, boletas, dinero);
chan recibo[p-1](vuelto, recibo);
process persona[id:0..P-1]{
  int cantBoletas=algo;
  bool esEmbarazada=algo;
  if esEmbarazada{
    send colaEmbarazadas(id, boletas, dinero);
  }
  else if cantBoletas<5{
    send colaPocasBoletas(id, boletas, dinero);
  }
  else send colaNormal(id, boletas, dinero);
  receive(recibo[id]);
}

process cajero{
  while true{
    if(!empty(colaEmbarazadas)){
      receive colaEmbarazadas(idPersona, boletas, dinero)
    }
    else if(empty(colaEmbarazadas) and !empty(colaPocasBoletas)){
      receive colaPocasBoletas(idPersona, boletas, dinero)
    }
    else if(empty(colaEmbarazadas) and empty(colaPocasBoletas) and !empy(colaNormal)){
      receive colaPocasBoletas(idPersona, boletas, dinero)
    }
    ticket,vuelto=cobrar(boletas,dinero);
    send recibo[idPersona], ticket, vuelto;
  }
}