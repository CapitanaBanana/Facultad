...
private Estrategia estrategia =Estratega.ZOMBIE;
public Ghost(double startingPointX, double startingPointY) {
    ...
    public void OnWallHit(){
        estrategia.changeDirection(this, mustChangeDirection);
    }
    @Override
    ...
    protected boolean outOfRange(){
        ...
        if (this.y + deltaY < 11){
            ...
            switch (i) {
                case 0:
                    deltaX = Direcciones.ABAJO.dx();;
                    deltaY = Direcciones.ABAJO.dy();

                    break;
                case 1:
                    deltaX = Direcciones.DERECHA.dx();
                    deltaY = Direcciones.DERECHA.dy();;
                    break;
                case 2:
                    deltaX = Direcciones.IZQUIERDA.dx();
                    deltaY = Direcciones.IZQUIERDA.dy();
                    break;
            }
        }
        return false;
	...
	private void setDirection(int Tx ,int Ty ,int Sx , int Sy) {
		...
		//turn up
		if (Ty>Sy){
            deltaX = Direcciones.ARRIBA.dx();
            deltaY = Direcciones.ARRIBA.dy();;
		}
			super.move();
